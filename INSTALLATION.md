...
> bibliotecas `.jar`. El archivo de datos se crea dinámicamente en un directorio de trabajo escribible (por defecto
> `%LOCALAPPDATA%\ControlGimnasio\database\gimnasio.db` en Windows, o el valor definido en la variable de entorno
> `CONTROL_GIMNASIO_HOME`). Por lo tanto, al instalar el sistema no necesitas copiar una base ya existente dentro del paquete.
> Solo asegúrate de que la aplicación tenga permisos para crearla en la primera ejecución.
