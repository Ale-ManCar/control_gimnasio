...
#### Cómo interpretar el archivo `logs/app.log`

Cuando instales versiones nuevas del ejecutable, revisa `logs/app.log` para confirmar qué runtime se ejecutó realmente. Verás pares de líneas como:

```
[2025-10-08 11:44:05] INFO: Iniciando procesos de arranque desde el splash
[2025-10-08 11:44:06] ERROR: Fallo durante el splash
java.lang.NoClassDefFoundError: java/sql/SQLException
```

seguidas más adelante por

```
[2025-10-08 11:53:56] INFO: Iniciando procesos de arranque desde el splash
[2025-10-08 11:53:58] INFO: Splash completado correctamente
[2025-10-08 11:53:58] INFO: Ventana de autenticación mostrada
```

Esto significa que primero se ejecutó una copia antigua del instalador (sin `java.sql`) y, después, la nueva versión generada con el script completo. Si la bitácora solo muestra entradas `INFO` de *Splash completado* y *Ventana de autenticación mostrada*, estás usando el runtime correcto. Si reaparece el error `java/sql/SQLException`, busca otros accesos directos o ejecutables antiguos y vuelve a lanzar el instalador regenerado.
